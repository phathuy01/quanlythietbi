...
<div id="addModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium text-gray-900">Thêm vị trí mới</h3>
            <button onclick="document.getElementById('addModal').classList.add('hidden')" class="text-gray-400 hover:text-gray-500">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        <form action="{{ route('vitri.create') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="ten_vt" class="block text-sm font-medium text-gray-700">Tên vị trí</label>
                <input type="text" name="ten_vt" id="ten_vt" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium text-gray-900">Chỉnh sửa vị trí</h3>
        ...
        <form id="editForm" method="POST">
            ...
            @method('PUT')
            <div class="mb-4">
                <label for="edit_ten_vt" class="block text-sm font-medium text-gray-700">Tên vị trí</label>
                <input type="text" name="ten_vt" id="edit_ten_vt" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <p class="text-gray-700 mb-4">Bạn có chắc chắn muốn xóa vị trí này?</p>
...
<div id="importModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <form action="{{ route('vitri.upload') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
...
<script>
    function openEditModal(id, ten_vt, mota) {
        document.getElementById('edit_ten_vt').value = ten_vt;
        document.getElementById('edit_mota').value = mota;
        document.getElementById('editForm').action = `/qlvt_upd/${id}`;
        document.getElementById('editModal').classList.remove('hidden');
    }

    function openDeleteModal(id) {
        document.getElementById('deleteForm').action = `/qlvt_del/${id}`;
